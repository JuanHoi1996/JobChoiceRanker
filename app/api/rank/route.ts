import {
  validateRankAnalysis,
  validateRankInput,
} from "../../../lib/rank-contract.js";
...
export async function POST(request: Request) {
  try {
    ...
    const input = validateRankInput(rawInput);
    const jobs = (
      input.jobs as {
        id: string;
        company: string;
    ...
    const analysis = validateRankAnalysis(
      (input.jobs as { id: string }[]).map((job) => job.id),
      JSON.parse(completion.text),
    );
    if (analysis.ranked.length !== (input.jobs as unknown[]).length)
